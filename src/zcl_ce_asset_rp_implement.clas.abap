CLASS zcl_ce_asset_rp_implement DEFINITION
  PUBLIC
  FINAL
...
CLASS zcl_ce_asset_rp_implement IMPLEMENTATION.
  ...
  METHOD get_giadauky.
    "Nguyên giá đầu kỳ A + Khấu hao đầu kỳ A
    ...
    "Nguyên giá đầu kỳ B + Khấu hao đầu kỳ B
  ...
  METHOD build_main_data.
    ...
    "3. Tính giá đầu kỳ (nguyên giá + khấu hao)
  ...
  METHOD get_result_giadauky.
    ...
    IF sy-subrc = 0.
      ...
      DATA(lv_khauhaodaukya)   = ls_nguyengiadaukya-khauhaodaukya.
    ENDIF.
    ...
    IF sy-subrc = 0.
      ...
      DATA(lv_khauhaodaukyb)   = ls_nguyengiadaukyb-khauhaodaukyb.
    ENDIF.

    cs_result-NguyenGiaDauKy = abs( lv_nguyengiadaukya + lv_nguyengiadaukyb ).
    cs_result-KhauHaoDauKy   = abs( lv_khauhaodaukya + lv_khauhaodaukyb ).
  ENDMETHOD.
  ...
  METHOD get_result_tanggiamgia.
    READ TABLE it_tanggiamgia INTO DATA(ls_tanggiamnguyengia) WITH KEY CompanyCode       = is_key-CompanyCode
                                                                       ...
                                                                       FiscalYear        = is_key-FiscalYear.
    IF sy-subrc = 0.
      cs_result-TangNguyenGia = abs( ls_tanggiamnguyengia-tangnguyengia ).
      cs_result-GiamNguyenGia = abs( ls_tanggiamnguyengia-GiamNguyenGia ).
      cs_result-TangKhauHao   = abs( ls_tanggiamnguyengia-TangKhauHao ).
  ...
  METHOD get_result_giacuoiky.
    "Nguyên giá cuối kỳ
    cs_result-NguyenGiaCuoiKy = cs_result-NguyenGiaDauKy + cs_result-TangNguyenGia - cs_result-GiamNguyenGia.

    "Khấu hao cuối kỳ
    cs_result-KhauHaoCuoiKy = cs_result-KhauHaoDauKy + cs_result-TangKhauHao - cs_result-GiamKhauHao.

    "Giá trị còn lại cuối kỳ
    cs_result-GiaTriConLaiCuoiKy = cs_result-NguyenGiaCuoiKy - cs_result-KhauHaoCuoiKy.
  ENDMETHOD.
  ...
  METHOD get_giadaukyb_final.
    ...
    FIELDS
        ...
        SUM( a~khauhaodaukyb ) AS khauhaodaukyb
    GROUP BY
        a~CompanyCode,
        a~DepreciationAreas,
        a~AssetNumber,
  ...
  METHOD get_tanggiamgia_final.
    ...
    FIELDS
        ...
        SUM( a~tangkhauhao ) AS tangkhauhao,
        SUM( a~giamkhauhao ) AS giamkhauhao
    GROUP BY
        a~CompanyCode,
        a~DepreciationAreas,
        a~AssetNumber,
